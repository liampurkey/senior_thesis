...
```{r}
post_data = nyc_data %>%
  spread.variables(variable = "d_od_lowwage", years = c(2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010), unit_var = "GEOID", time_var = "year", data = .) %>%
  ...
  spread.variables(variable = "d_od_highwage", years = c(2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010), unit_var = "GEOID", time_var = "year", data = .) %>%
  filter(year >= 2011) %>%
  group_by(GEOID) %>%
  mutate(d_od_lowwage_post = mean(d_od_lowwage)) %>%
  mutate(d_od_midwage_post = mean(d_od_midwage)) %>%
  mutate(d_od_highwage_post = mean(d_od_highwage)) %>%
  ungroup() %>%
  filter(year == 2018)
```
...
```{r}
lw = sdd.estimate(outcome = "d_od_lowwage", treatment_var = "edugentrify", unit_var = "GEOID", time_var = "year", cluster_var = "NTA", 
...
```{r}
lwlasso = rlasso(d_od_lowwage_post ~ p_white_2010 + p_black_2010 + p_hispanic_2010 + p_asian_2010 + p_hs_pop_2010 +
                           medianincome_2010 + medianrent_2010 + dist_cbd + dist_hi +
                           p_nhood_college_pop_2010 + nta_medianincome_2010 + 
                           p_lesshs_pop_2010 + p_lesshs_laborforce_2010 + p_lesshs_unemp_2010 + 
                           p_hs_pop_2010 + p_hs_laborforce_2010 + p_hs_unemp_2010 + 
                           d_od_lowwage_2002 + d_od_lowwage_2003 + d_od_lowwage_2004 + d_od_lowwage_2005 + d_od_lowwage_2006 + 
                           d_od_lowwage_2007 + d_od_lowwage_2009 + d_od_lowwage_2010, 
...
```{r}
lw_covs = sdd.estimate(outcome = "d_od_lowwage", treatment_var = "edugentrify", unit_var = "GEOID", time_var = "year", cluster_var = "NTA", 
...
```{r}
hw = sdd.estimate(outcome = "d_od_highwage", treatment_var = "edugentrify", unit_var = "GEOID", time_var = "year", cluster_var = "NTA", 
...
```{r}
hwlasso = rlasso(d_od_highwage_post ~ p_white_2010 + p_black_2010 + p_hispanic_2010 + p_asian_2010 + p_hs_pop_2010 +
                           medianincome_2010 + medianrent_2010 + dist_cbd + dist_hi +
                           p_nhood_college_pop_2010 + nta_medianincome_2010 + 
                           p_lesshs_pop_2010 + p_lesshs_laborforce_2010 + p_lesshs_unemp_2010 + 
                           p_hs_pop_2010 + p_hs_laborforce_2010 + p_hs_unemp_2010 + 
                           d_od_highwage_2002 + d_od_highwage_2003 + d_od_highwage_2004 + d_od_highwage_2005 + d_od_highwage_2006 + 
                           d_od_highwage_2007 + d_od_highwage_2009 + d_od_highwage_2010, 
                           data = post_data, intercept = FALSE, post = FALSE)
```
...
```{r}
hw_covs = sdd.estimate(outcome = "d_od_highwage", treatment_var = "edugentrify", unit_var = "GEOID", time_var = "year", cluster_var = "NTA", 
...
print(textable, file = "/Users/liampurkey/Desktop/Honors/Results/Tables/New York/Tex/od_wage_main.tex", sanitize.text.function = function(x) x)
```
...
lw_synth = inner_join(lw_plot, lw_covs_plot %>% select(-time_weight, -treated), by = "year") %>%
  ...
  xlab("Year") +
  ylim(c(150,550)) +
  annotate(geom="text", x=2006, y=500, label="Onset of Gentrification",
...
lw_raw = lw_lasso_data %>% 
  ...
  summarize(value = mean(d_od_lowwage)) %>%
...
```{r}
ggsave("/Users/liampurkey/Desktop/Honors/Results/Plots/New York/od_lw_main.png", width = 8, height = 4, units = "in")
```
...
mw_raw = mw_lasso_data %>% 
  group_by(edugentrify, year) %>%
  summarize(value = mean(d_od_midwage)) %>%
  mutate(variable = case_when(edugentrify == 1 ~ "Gentrifying",
                              edugentrify == 0 ~ "Not Gentrifying")) %>%
  ggplot(aes(x = year, y = value)) + 
  geom_line(aes(color = variable, linetype = variable)) + 
...
```{r}
ggsave("/Users/liampurkey/Desktop/Honors/Results/Plots/New York/od_mw_main.png", width = 8, height = 4)
```
...
hw_synth = inner_join(hw_plot, hw_covs_plot %>% select(-time_weight, -treated), by = "year") %>%
  ...
  xlab("Year") +
  ylim(c(50,250)) +
  annotate(geom="text", x=2006, y=200, label="Onset of Gentrification",
              color="black", family = "Palatino", size = 3) +
  theme(panel.grid = element_blank(),
        panel.background = element_blank(),
        strip.text = element_blank(),
...
hw_raw = hw_lasso_data %>% 
  group_by(edugentrify, year) %>%
  summarize(value = mean(d_od_highwage)) %>%
  mutate(variable = case_when(edugentrify == 1 ~ "Gentrifying",
                              edugentrify == 0 ~ "Not Gentrifying")) %>%
  ggplot(aes(x = year, y = value)) + 
  geom_line(aes(color = variable, linetype = variable)) + 
...
```{r}
ggsave("/Users/liampurkey/Desktop/Honors/Results/Plots/New York/od_hw_main.png", width = 8, height = 4)
```
...
```{r}
write_csv(wage_weights, "/Users/liampurkey/Desktop/Honors/Results/Tables/New York/Raw/od_wage_ntas.csv")
```
